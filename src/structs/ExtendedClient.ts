...
const fileFilter = (fileName: string) => fileName.endsWith(".ts") || fileName.endsWith(".js");

export class ExtendedCient extends Client {
    ...
    private registerModules() {
        ...
        fs.readdirSync(commandsPath).forEach(local => {
            fs.readdirSync(commandsPath + `/${local}/`).filter(fileFilter).forEach(async fileName => {
    ...
    private registerEvents(){
        ...
        fs.readdirSync(eventsPath).forEach(local => {
            ...
            fs.readdirSync(`${eventsPath}/${local}`).filter(fileFilter)
